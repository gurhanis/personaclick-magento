 ...
 */
namespace Personaclick\Personalization\Controller\Feed;
...
class Xml extends \Magento\Framework\App\Action\Action
{
        ...
        if ($this->_config->isPersonaclickEnabled()) {
            if ($this->get('step')) {
                if ($this->get('step') == 1) {
                    ...
                    return $this->resultRedirectFactory->create()->setPath('personaclick/feed/xml', ['_query' => ['step' => '2']]);
                } elseif ($this->get('step') == 2) {
                    $this->generateCategories();

                    $this->recorder('    <offers>' . "\n", 'a');

                    return $this->resultRedirectFactory->create()->setPath('personaclick/feed/xml', ['_query' => ['start' => 'start']]);
                }
                ...
                if (is_file($this->fileCron()) && $this->_config->isCronEnabled()) {
                    ...
                    return $this->resultRedirectFactory->create()->setPath('personaclick/feed/xml', ['_query' => ['step' => '1']]);
                }
            }
        } else {
    ...
    {
        return $this->_data->getDir('media') . '/personaclick.xml';
    }

    protected function fileCron()
    {
        return $this->_data->getDir('media') . '/personaclick_cron.xml';
    }
    ...
    protected function recorder($xml, $mode)
    {
        if (!$fp = fopen($this->file(), $mode)) {
            $this->_logger->log('PersonaClick: could not open xml file');
        } elseif (fwrite($fp, $xml) === false) {
            $this->_logger->log('PersonaClick: XML file not writable');
        }
